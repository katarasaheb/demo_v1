...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const fadeInEffect = () => {
        ...
        // Add fade-in and move effect as the user scrolls
        if (scrollPosition > heroHeight * 0.2) {
            hero.classList.add("fade-in");
        } else {
            hero.classList.remove("fade-in");
        ...
        // You can also add parallax-like movement
...
document.addEventListener('DOMContentLoaded', function () {
    // Trigger smooth scroll for anchors if needed
    const scrollToTop = document.querySelector('#hero');
    scrollToTop.scrollIntoView({
        behavior: 'smooth'
    });
});

// Form Validation Example (ensuring required fields are filled)
const form = document.querySelector("form");
form.addEventListener("submit", function (e) {
    const name = document.getElementById("name");
    const email = document.getElementById("email");
